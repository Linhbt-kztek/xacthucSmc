					        ...
					        <div class="col-md-6">
					        	<div class="form-group {{$errors->has('company_id') ? 'has-error' : ''}}">
						      	...
						      	<div class="form-group {{$errors->has('name') ? 'has-error' : ''}}">
					              	...
					              	<input type="text" class="form-control" name="name" id="name" value="{{$model->name}}" placeholder="Nhập tên sản phẩm" maxlength="255" required>
					              	<span class="help-block">{{$errors->first("name")}}</span>
					            </div>
					            <!-- /.form-group -->
						      	<div class="form-group {{$errors->has('code') ? 'has-error' : ''}}">
					              	<label class="required" for="code">Barcode - Mã sản phẩm </label>
					              	<input type="text" class="form-control" name="code" id="code" placeholder="Mã sản phẩm" value="{{$model->code}}" maxlength="50" required>
					              	<span class="help-block">{{$errors->first('code')}}</span>
					            </div>
					             <!-- /.form-group -->
						      	<div class="form-group {{$errors->has('batchcode') ? 'has-error' : ''}}">
					              	<label class="" for="code">Batchcode - Mã lô sản phẩm </label>
					              	<input type="text" class="form-control" name="batchcode" id="bathcode" placeholder="Mã batchcode" value="{{$model->batchcode}}" maxlength="50">
					              	<span class="help-block">{{$errors->first('bachcode')}}</span>
					            </div>
					            ...
					            <div class="form-group {{$errors->has('date_off') ? 'has-error' : ''}}">
					              	...
					              	<input type="date" class="form-control" name="date_off" id="date_off" value="{{$model->date_off}}">
					              	<span class="help-block">{{$errors->first("name")}}</span>
					            </div>
					            <!-- /.form-group -->
					            <!--
					            <div class="form-group {{$errors->has('date_output') ? 'has-error' : ''}}">
					              	...
					              	<input type="time" class="form-control" name="time_out" id="time_out" value="{{$model->time_out}}">
					              	<span class="help-block">{{$errors->first("name")}}</span>
					            </div>
					            -->
					            <!-- /.form-group -->
					            ...
					            <div class="form-group {{$errors->has('date_output') ? 'has-error' : ''}}">
					              	...
					              	<input type="text" class="form-control" name="price" id="time_out" value="{{$model->price}}">
					              	<span class="help-block">{{$errors->first("name")}}</span>
					            </div>
					            
					             <div class="form-group {{$errors->has('code') ? 'has-error' : ''}}">
            		              	<label class="" for="code">Hiện thông tin mã tem khi quét </label>
            		              <input type="checkbox" name="serialshow" value="1" {{$model->serialshow==1 ? 'checked' : ''}} >
					            ...
					            <div class="form-group {{$errors->has('code') ? 'has-error' : ''}}">
            		              ...
            		              <input type="checkbox" name="formactive" value="1" {{$model->formactive==1 ? 'checked' : ''}} >
            		            ...
            		            <div class="form-group {{$errors->has('code') ? 'has-error' : ''}}">
            		              	...
            		              	<input type="text" class="form-control" name="formlabel" id="code" placeholder="Nhập tiêu đề form" value="{{$model->formlabel}}" maxlength="250">
            		              	<span class="help-block">{{$errors->first('code')}}</span>
            		            </div>
            		            <!-- /.form-group -->
            		            <!-- /.form-group -->
            		            <div class="form-group {{$errors->has('code') ? 'has-error' : ''}}">
            		              	...
            		              	<textarea name="formnote1" id="formnote1" class="form-control" rows="4"> {{$model->formnote}} </textarea>
            		            ...
            		            <!-- /.form-group -->
						      	  ...
						      	  <div class="form-group {{$errors->has('intro') ? 'has-error' : ''}}">
					              	<label class="required" for="intro">Tạo số dự thưởng (1=Có ; 0= Không)</label>
					              	 	<input type="number" class="form-control" name="reward" id="reward" value="{{$model->reward}}" maxlength="1" min="0" max="1" required>
					          	...
					          	<div class="form-group {{$errors->has('introimage') ? 'has-error' : ''}}">
				                  	...
				                  	<p class="mutiple_image">
				                  		<span class="imgs">
				                  			@foreach($model->product_image as $img)
				                  			<img src="{{asset($img->path)}}" alt="" style="max-width: 25%;margin-top: 10px;">
				                  			@endforeach
				                  		</span>
				                  	</p>
			      	...
			      	<h3> THAY ĐỔI NỘI DUNG HIỆN THỊ MẶC ĐỊNH</h3>
			      	            <div class="form-group {{$errors->has('intro') ? 'has-error' : ''}}">
					              	<label class="required" for="intro">Thông báo kết quả xác thực</label>
					              	 	<input type="text" class="form-control" name="alertmessage" id="alertmessage" value="{{$model->alertmessage}}" >
					            </div>
					            <div class="form-group {{$errors->has('intro') ? 'has-error' : ''}}">
					              	<label class="required" for="intro">Tiêu đề thông tin sản phẩm</label>
					              	 	<input type="text" class="form-control" name="productinfo" id="productinfo" value="{{$model->productinfo}}" >
					            </div>
					            <div class="form-group {{$errors->has('intro') ? 'has-error' : ''}}">
					              	<label class="required" for="intro">Thông tin doanh nghiệp sở hữu</label>
					              	 	<input type="text" class="form-control" name="companyinfo" id="companyinfo" value="{{$model->companyinfo}}" >
					            </div>
					            <div class="form-group {{$errors->has('intro') ? 'has-error' : ''}}">
					              	<label class="required" for="intro">Thông tin nhà phân phối, đối tác, nhập khẩu</label>
					              	 	<input type="text" class="form-control" name="parnerinfo" id="parnerinfo" value="{{$model->parnerinfo}}" >
					            </div>
			      	<!-- /.form-group -->
