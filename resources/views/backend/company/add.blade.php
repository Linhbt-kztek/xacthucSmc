...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<form action="{{route('backend.company.pAdd')}}" method="POST" enctype="multipart/form-data">
				{{csrf_field()}}
				      	...
				      	<div class="row">
					        <div class="col-md-6 col-sm-6 col-xs-12">
					        	...
					        	<div class="form-group {{$errors->has('name') ? 'has-error' : ''}}">
					              	...
					              	<input type="text" class="form-control" name="name" id="name" value="{{old('name')}}" placeholder="Tên doanh nghiệp" maxlength="255">
					            ...
					            <div class="form-group {{$errors->has('tel') ? 'has-error' : ''}}">
					             ...
					             <div class="form-group {{$errors->has('tel') ? 'has-error' : ''}}">
					              	...
					              	<input type="text" class="form-control" name="facebooklink" id="tel" placeholder="Link facebook" value="{{old('facebooklink')}}" maxlength="150">
					              	<span class="help-block">{{$errors->first("facebooklink")}}</span>
					            </div>
					            <!-- /.form-group -->
					            <div class="form-group {{$errors->has('address') ? 'has-error' : ''}}">
					              	<label class="required" for="address">Địa chỉ</label>
					              	<textarea class="form-control" name="address" rows="3" id="address" placeholder="Địa chỉ" maxlength="1155">{{old('address')}}</textarea>
					              	<span class="help-block">{{$errors->first("address")}}</span>
					            </div>
					            <!-- /.form-group -->
					            <div class="form-group {{$errors->has('code_tax') ? 'has-error' : ''}}">
					              	<label class="" for="code_tax">Mã số thuế</label>
					              	<input type="text" class="form-control" name="code_tax" id="code_tax" placeholder="Mã số thuế" value="{{old('code_tax')}}" maxlength="50">
					              	<span class="help-block">{{$errors->first("code_tax")}}</span>
					            </div>
					            <!-- /.form-group -->
					            <div class="form-group {{$errors->has('website') ? 'has-error' : ''}}">
					              	<label class="" for="website">Website</label>
					              	<input type="text" class="form-control" name="website" id="website" placeholder="Website" value="{{old('website')}}" maxlength="255">
					              	<span class="help-block">{{$errors->first("website")}}</span>
					            </div>
					            <!-- /.form-group -->
					            <div class="form-group {{$errors->has('intro') ? 'has-error' : ''}}">
					              	<label class="" for="intro">Giới thiệu công ty</label>
					              	<textarea class="form-control" name="intro" rows="3" id="intro" placeholder="Giới thiệu công ty">{{old('intro')}}</textarea>
				                ...
				                <div class="form-group {{$errors->has('asign_to') ? 'has-error' : ''}}">
						            ...
						            <select class="form-control" id="asign_to" name="asign_to">
						                ...
						                <option value="{{$item->id}}" {{(old('asign_to') == $item->id) ? 'selected' : '' }}>{{$item->email}}</option>
						                @endforeach
						            </select>
