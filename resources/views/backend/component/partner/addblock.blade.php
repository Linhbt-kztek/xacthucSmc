<form id="form-saveBlockPartner" action="{{route('backend.qrcode.saveBlockPartner')}}" method="POST">
  ...
  <div class="box box-primary">
      <div class="box-header with-border">
        ...
        <h3 class="text-center help">{{$block->company->name}} <a target="_blank" href="{{route('backend.company.vEdit',['id'=>$block->company->id])}}" class="editItem" id="" title="Sửa"><i class="fa fa-fw fa-edit"></i></a> | GUID: {{$block->guid}} | Serial: [{{$block->start}}-{{$block->end}}]</h3>
                  ...
                  <tbody>
                    ...
                    <tr role="row" id="partner-{{$partner->id}}" class="{{$key%2 == 0 ? 'even' : 'odd'}}">
                      ...
                      <td style="text-align: left !important;">
                          <a href="{{route('backend.partner.vEdit',['id'=>$partner->id])}}" class="editItem" id="" title="Sửa"><i class="fa fa-fw fa-edit"></i></a>
                          {{$partner->name}} ( {!!$partner->active==1 ? '<i class="fa fa-eye" aria-hidden="true" style="color: #3a52ff;"></i> hiện' : '<i class="fa fa-eye-slash" aria-hidden="true" style="color: red"></i> ẩn'!!} )  
                      </td>
