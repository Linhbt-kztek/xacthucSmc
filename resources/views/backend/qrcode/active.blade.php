...
@section('content')
    ...
    <section class="content-header">
        <h1>
            Lịch sử quét
            <small>{{ isset($company) ? $company->name : 'Quản lý' }}</small>
        </h1>
        <ol class="breadcrumb">
            ...
            <li class="active">Lịch sử quét
                {{ isset($filter['guid']) ? 'GUID: ' . substr($filter['guid'], 0, 8) . '...' : '' }}
            </li>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="box box-primary">
                            ...
                            <div class="col-sm-8">
                                ...
                                @if (Auth::user()->hasAnyRole([1, 2]))
                                    <button class="btn btn-primary mrg-r-10"
                                        onclick="previewQrcodeForm('{{ route('backend.qrcode.previewQrcode') }}')">Tạo mới
                                        kích hoạt</button>
                    ...
                    <!-- /.box-header -->
                    <form id="search-form" action="{{ route('backend.qrcode.active') }}" method="GET">
                        <div class="box box-default collapsed-box" id="box-search">
                            ...
                            <div class="box-body">
                                <input type="hidden" id="guid_active_id" name="guid"
                                    value="{{ isset($filter['guid']) ? $filter['guid'] : '' }}">
                                            ...
                                            <select class="form-control col-md-6" id="company_id" name="company_id"
                                                ...
                                                @foreach ($listCompany as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ isset($filter['company_id']) && $filter['company_id'] == $item->id ? 'selected' : '' }}>
                                                        {{ $item->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                            ...
                                            <select class="form-control" name="partner_id" id="partner_id"
                                                ...
                                                @if (isset($listPartner))
                                                    @foreach ($listPartner as $item)
                                                        <option value="{{ $item->id }}"
                                                            {{ isset($filter['partner_id']) && $filter['partner_id'] == $item->id ? 'selected' : '' }}>
                                                            {{ $item->text }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="number" name="serial"
                                                value="{{ isset($filter['serial']) ? $filter['serial'] : '' }}"
                                                class="form-control">
                                        </div>
                                        <!-- /.form-group -->
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="product_name"
                                                value="{{ isset($filter['product_name']) ? $filter['product_name'] : '' }}"
                                                class="form-control">
                                        </div>
                                        <!-- /.form-group -->
                                    </div>
                    ...
                    <div class="modal-footer">
                        <center>
                            <button type="button" class="btn btn-primary"
                                onclick="destroyOrActiveMutipleQrcode(1, '{{ route('backend.qrcode.destroyOrActiveMutipleQrcode') }}')">Lưu
                                kích hoạt</button>
                            <button type="button" class="btn btn-danger"
                                onclick="destroyOrActiveMutipleQrcode(2, '{{ route('backend.qrcode.destroyOrActiveMutipleQrcode') }}')">Hủy
                                kích hoạt</button>
                        </center>
                    </div>
                </div>
