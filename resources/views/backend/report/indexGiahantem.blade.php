	...
	<div class="row">
		<div class="col-sm-12">
			<div class="box box-primary">
				...
				<!-- /.box-header -->
				<!-- search form -->
				<form id="search-form" action="{{route('backend.report.indexGiahantem')}}" method="GET">
					            ...
					            <div class="col-md-6">
					              <div class="form-group">
					                ...
					                <select class="form-control" name="protected_time_of_tem" id="protected_time_of_tem_search" style="width: 100%;">
					                  	...
					                  	@for($i=2;$i<37;$i++)
			                            <option value="{{$i}}" {{(isset($filter['protected_time_of_tem']) && $filter['protected_time_of_tem'] == $i) ? 'selected' : '' }}>{{$i.' tháng'}}</option>
			                            @endfor
					                </select>
					              </div>
					              <!-- /.form-group -->
				        				...
				        				@forelse ($listProduct as $key => $item) 
				              			<tr class="{{$key%2 == 0 ? 'even' : 'odd'}}">
			              					<td class="text-center">
						                        @if(isset($item->product) && $item->product->introimage != '')
						                        <img src="{{asset($item->product->introimage)}}" alt="" style="width: 50px; height: 50px">
						                        @endif
					                      	</td>
					                      	<td class="text-left">{{isset($item->product) ? $item->product->code : ''}}</td>
					                      	<td class="text-left">{{isset($item->product) ? $item->product->name : ''}}</td>
					                      	<td class="text-left">{{isset($item->product) ? $item->product->protected_time.' tháng' : ''}}</td>
					                      	<td class="text-left">{{$item->created_at}}</td>
					                      	<td>{{$item->start}}</td>
					                      	<td>{{$item->amount}}</td>
					                      	<td>{{$item->end}}</td>
					                      	<td class="text-center">
						                        <select class="form-control" id="protected_time_of_tem_{{$item->product_id}}">
						                            @for($i=1;$i<100;$i++)
						                            <option value="{{$i}}" {{$item->protected_time_of_tem == $i ? 'selected' : ''}}>{{$i.' tháng'}}</option>
						                            @endfor
						                        </select>
					                      	</td>
								          	<td class="text-center">
								          		<a href="javascript: void(0);" data-href="{{route('backend.report.refreshTime')}}" class="editItem refreshTime" data-productid="{{$item->product_id}}" data-companyid="{{isset($item->company) ? $item->company->id : ''}}" data-guid="{{$item->guid}}" title="Gia hạn tem"><i class="fa fa-refresh"></i></a>
								          	</td>
								        </tr>
								        @empty
								    	<tr class="even">
